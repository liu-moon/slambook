...
public:
  ...
  : Node("orb_feature_extraction_node"),first_image_path_(img1_path), second_image_path_(img2_path) {
    ...
    second_image_ = cv::imread(second_image_path_, cv::IMREAD_GRAYSCALE);
    if (first_image_.empty() || second_image_.empty()) {
...
private:
  void process_images() {
    ...
    ComputeORB(first_image_, keypoints1, descriptors1);
    ComputeORB(second_image_, keypoints2, descriptors2);

    // 输出ORB特征提取耗时
    RCLCPP_INFO(this->get_logger(), "ORB特征提取完成");
    ...
    // 输出匹配耗时及结果
    RCLCPP_INFO(this->get_logger(), "ORB匹配完成，共找到 %zu 对匹配", matches.size());

    // 显示匹配结果
    cv::Mat img_show;
    cv::drawMatches(first_image_, keypoints1, second_image_, keypoints2, matches, img_show);
  ...
  void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
    ...
    RCLCPP_INFO(this->get_logger(), "bad/total: %d/%zu", bad_points, keypoints.size());
  }
  ...
  string first_image_path_, second_image_path_;
